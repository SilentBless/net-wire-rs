//! Exact mutable KCP segment views with fixed structural boundaries.

use super::{
    KCP_SEGMENT_HEADER_LEN, KcpCommand, KcpConversationId, KcpFragment, KcpSegmentParseError,
...
impl<'a> KcpSegmentMut<'a> {
    ...
    pub fn parse(bytes: &'a mut [u8]) -> Result<(Self, &'a mut [u8]), KcpSegmentParseError> {
        let (segment, _) = super::KcpSegment::parse(bytes)?;
