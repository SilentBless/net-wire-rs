...
pub(crate) fn encoded_len(decoded: &[u8]) -> Result<usize, EncodeError> {
    let mut bits = 0usize;
    for &byte in decoded {
        bits = bits
            .checked_add(usize::from(CODES[usize::from(byte)].1))
            .ok_or(EncodeError::BitLengthOverflow)?;
    }
    bits.checked_add(7)
        .ok_or(EncodeError::ByteLengthOverflow)
        .map(|bits| bits / 8)
}
pub(crate) fn encode_prevalidated(decoded: &[u8], destination: &mut [u8]) {
    let mut pending = 0u64;
    let mut pending_bits = 0u8;
    let mut output = 0usize;
    for &byte in decoded {
        let (code, code_bits) = CODES[usize::from(byte)];
...
pub(crate) fn decoded_len(encoded: &[u8]) -> Result<usize, DecodeError> {
    let mut code = 0u32;
    let mut bits = 0u8;
    ...
    for byte in encoded {
        for shift in (0..8).rev() {
            ...
            if let Some(symbol) = symbol_for(code, bits) {
                if symbol == 256 {
                    return Err(DecodeError::EosSymbol);
                }
                length = length
                    .checked_add(1)
                    .ok_or(DecodeError::DecodedLengthOverflow)?;
                code = 0;
                bits = 0;
...
pub(crate) fn decode_prevalidated(encoded: &[u8], destination: &mut [u8]) {
    ...
    for byte in encoded {
        for shift in (0..8).rev() {
            ...
            if let Some(symbol) = symbol_for(code, bits) {
                if symbol != 256 {
                    destination[output] = symbol as u8;
                    output += 1;
                }
                code = 0;
                bits = 0;
...
fn is_prefix(code: u32, bits: u8) -> bool {
    CODES.iter().any(|&(candidate, candidate_bits)| {
        candidate_bits > bits && (candidate >> (candidate_bits - bits)) == code
    })
}
fn validate_tail(code: u32, bits: u8) -> Result<(), DecodeError> {
    if bits == 0 {
        return Ok(());
    }
