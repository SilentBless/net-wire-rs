...
This is one package and one crate, using Rust 1.91 and edition 2024. It has `#![no_std]`, `#![deny(missing_docs)]`, `unsafe_code = deny`, an empty default feature set, no production allocation, and no production I/O. `wire-repr` is a foundational production dependency that owns physical representation where adopted; it is not optional or protocol-scoped.
